...
fn group_searchable_sessions(
    ...
    for (record, assistant_type) in entries {
        ...
        if entry.agent_nickname.is_some() {
            session.agent_nickname = entry.agent_nickname;
        }
...
pub async fn search_sessions_by_user_prompt(
    ...
    let search_result: Result<SessionSearchResponse, String> = tokio::task::spawn_blocking({
        ...
        move || {
            ...
            let sessions = group_searchable_sessions(entries);
...
mod tests {
    ...
    fn prompt_search_keeps_same_session_id_source_directories_separate() {
        ...
        let mut source_a = legacy_usage_entry(1, "gpt-5", tokens);
        source_a.session_id = "shared-search-session".to_string();
        source_a.source_kind = Some("copilot-app".to_string());
        source_a.source_dir_key = Some("aa".to_string());
        let mut source_b = source_a.clone();
        source_b.source_dir_key = Some("bb".to_string());

        let sessions = group_searchable_sessions(vec![
            usage_day_record(source_a, "copilot"),
            usage_day_record(source_b, "copilot"),
        ]);
